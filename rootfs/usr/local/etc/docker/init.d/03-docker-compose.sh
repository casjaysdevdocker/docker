...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410201142-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  zz-docker-compose.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Sunday, Oct 20, 2024 11:42 EDT
# @@File             :  zz-docker-compose.sh
# @@Description      :
...
DATA_DIR="/data/docker-compose"   # set data directory
CONF_DIR="/config/docker-compose" # set config directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
VAR_DIR=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TMP_DIR="/tmp/docker-compose"       # set the temp dir
RUN_DIR="/run/docker-compose"       # set scripts pid dir
LOG_DIR="/data/logs/docker-compose" # set log directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set the working dir
WORK_DIR=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
RUNAS_USER="docker" # normally root
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
root_user_name="${DOCKER_COMPOSE_ROOT_USER_NAME:-}" # root user name
root_user_pass="${DOCKER_COMPOSE_ROOT_PASS_WORD:-}" # root user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
user_name="${DOCKER_COMPOSE_USER_NAME:-}"      # normal user name
user_pass="${DOCKER_COMPOSE_USER_PASS_WORD:-}" # normal user password
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
[ -f "/config/env/docker-compose.script.sh" ] && . "/config/env/docker-compose.script.sh" # Generated by my dockermgr script
[ -f "/config/env/docker-compose.sh" ] && . "/config/env/docker-compose.sh"               # Overwrite the variabes
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
if [ -z "$COMPOSE_FILE" ] && [ ! -f "$COMPOSE_FILE" ]; then
  [ -f "/config/compose.yml" ] && COMPOSE_FILE="/config/compose.yml"
  [ -f "/config/compose.yaml" ] && COMPOSE_FILE="/config/compose.yaml"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__update_conf_files() {
  ...
  # - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
  # replace variables
  # __replace "" "" "$CONF_DIR/docker-compose.conf"
...
__post_execute() {
  ...
  (
    ...
    if [ -n "$COMPOSE_FILE" ]; then
      __cd "/data/compose"
      ln -sf "$COMPOSE_FILE" "./compose.yaml" && docker compose up
    fi
    # show exit message
    __banner "$postMessageEnd: Status $retVal"
  ) 2>"/dev/stderr" | tee -p -a "$LOG_DIR/app.log" &
  pid=$!
...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# root/admin user info [password/random]
#ENV_ROOT_USER_NAME="${ENV_ROOT_USER_NAME:-$DOCKER_COMPOSE_ROOT_USER_NAME}"   # root user name
#ENV_ROOT_USER_PASS="${ENV_ROOT_USER_NAME:-$DOCKER_COMPOSE_ROOT_PASS_WORD}"   # root user password
#root_user_name="${ENV_ROOT_USER_NAME:-$root_user_name}"                              #
#root_user_pass="${ENV_ROOT_USER_PASS:-$root_user_pass}"                              #
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
#Normal user info [password/random]
#ENV_USER_NAME="${ENV_USER_NAME:-$DOCKER_COMPOSE_USER_NAME}"                  #
#ENV_USER_PASS="${ENV_USER_PASS:-$DOCKER_COMPOSE_USER_PASS_WORD}"             #
