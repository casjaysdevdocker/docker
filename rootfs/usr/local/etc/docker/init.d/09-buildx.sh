...
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
##@Version           :  202410201150-git
# @@Author           :  Jason Hempstead
# @@Contact          :  jason@casjaysdev.pro
# @@License          :  WTFPL
# @@ReadME           :  09-buildx.sh --help
# @@Copyright        :  Copyright: (c) 2024 Jason Hempstead, Casjays Developments
# @@Created          :  Sunday, Oct 20, 2024 11:50 EDT
# @@File             :  09-buildx.sh
# @@Description      :
...
# set data directory
DATA_DIR="/data/buildx"
# set config directory
CONF_DIR="/config/buildx"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
# root user name
root_user_name="${BUILDX_ROOT_USER_NAME:-}"
# root user password
root_user_pass="${BUILDX_ROOT_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Normal user info [password/random]
# normal user name
user_name="${BUILDX_USER_NAME:-}"
# normal user password
user_pass="${BUILDX_USER_PASS_WORD:-}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Load variables from config
# Generated by my dockermgr script
[ -f "/config/env/buildx.script.sh" ] && . "/config/env/buildx.script.sh"
# Overwrite the variabes
[ -f "/config/env/buildx.sh" ] && . "/config/env/buildx.sh"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__post_execute() {
	...
	(
		...
		# commands to execute
		__cd "$DATA_DIR"
		if [ -f "$RUN_DIR/buildx_cli.pid" ]; then
			echo "buildx is already running" && exit
		fi
		echo "Setting the build dir to $PWD"
		local prev_dir="$PWD"
		while :; do
			echo "$$" >"$RUN_DIR/buildx_cli.pid"
			find_dockerfiles="$(find "$DATA_DIR" -name 'Dockerfile*' 2>/dev/null | grep -v '^$' | sort -uV || false)"
			if [ -n "$find_dockerfiles" ]; then
				...
				echo "Can not find any dockerfiles" >&2
				echo "Mount your directory to $DATA_DIR and it will find and build all dockerfiles" >&2
				sleep 300
			fi
		done
		# show exit message
