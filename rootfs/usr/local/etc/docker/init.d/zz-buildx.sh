...
DATABASE_DIR="${DATABASE_DIR_BUILDX:-$DATABASE_BASE_DIR/sqlite}"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# Set webroot
...
VAR_DIR=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TMP_DIR="/tmp/buildx"       # set the temp dir
...
[ -f "/config/env/buildx.script.sh" ] && . "/config/env/buildx.script.sh" # Generated by my dockermgr script
[ -f "/config/env/buildx.sh" ] && . "/config/env/buildx.sh"               # Overwrite the variabes
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
__post_execute() {
  ...
  (
    ...
    # commands to execute
    __cd "$DATA_DIR"
    [ -f "$RUN_DIR/buildx_cli.pid" ] && exit
    echo "Setting the build dir to $PWD"
    local prev_dir="$PWD"
    while :; do
      echo "$$" >"$RUN_DIR/buildx_cli.pid"
      find_dockerfiles="$(find "$DATA_DIR" -name 'Dockerfile*' 2>/dev/null | grep -v '^$' | sort -uV | grep '^Dockerfile' || false)"
